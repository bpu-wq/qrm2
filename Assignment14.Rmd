...
author: "Group 44: Bianca Pariamachi"
...
```{r}
movies1 <- read.delim("movies1.tsv")
...
The dataset contains 505 observations and 19 variables. There are 505 subjects, where each subject is one movie. Therefore, each row represents one movie and the columns contain characteristics of that movie, such as its budget, revenue, runtime, genre, and average vote.
...
The dataset contains nominal, interval, and ratio variables. An example of a nominal variable is genre, since genres are categories without a natural order. An example of an interval variable is release_year, since differences between years are meaningful but there is no true zero. An example of a ratio variable is budget, since it has a meaningful zero and ratios are meaningful. There is no clear ordinal variable in the dataset because none of the categorical variables have a natural ranking.
...
mean(movies1$profits, na.rm = TRUE)
median(movies1$profits, na.rm = TRUE)
max(movies1$profits, na.rm = TRUE)
min(movies1$profits, na.rm = TRUE)

```

**Your Answer:**

The mean profit is approximately $63.12 million, while the median profit is $1.90 million. The maximum profit is approximately $2.55 billion, and the minimum profit is -$90 million.
...
The movie with the highest profit is Avatar, with a profit of $2,550,965,087. The movie with the lowest profit is Mighty Joe Young, with a profit of -$90,000,000.

*Step c*

```{r}

boxplot(movies$profits,
        main = "Distribution of Movie Profits",
        xlab = "Movies",
        ylab = "Profit ($)")
quantile(movies$profits)

```

**Your Answer:**

Q1 is $0, Q2 is $1.9 million, Q3 is approximately $60.51 million, and Q4 is approximately $2.55 billion. The distribution is strongly right-skewed. Most movies make relatively small profits compared with a small number of movies that make extremely large profits. This suggests that making money in the movie industry is uneven, with a few highly profitable movies having a large effect on overall profits.
...
When taking the log of profits, movies with low profits
